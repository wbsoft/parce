...
class Transformer:
    ...
    def transform_text(self, root_lexicon, text, pos=0):
        ...
        for e in events:
            ...
            if len(e.tokens) > 1:
                tokens = tuple(parce.tree._GroupToken(None, *t) for t in e.tokens)
                for t in tokens:
                    t.group = tokens
                items.extend(Item(None, None, t) for t in tokens)
            else:
                items.append(Item(None, None, parce.tree.Token(None, *e.tokens[0])))
        # unwind
        while True:
            if curlang is not lexicon.language:
                curlang = lexicon.language
